...
## 주의: 파일 경로 문제 — `%cd` 대신 터미널처럼 실행할 것

`%cd`로 이동하면 그 위치가 노트북 커널에 계속 남아있어서, 셀을 순서대로 안 돌리거나 압축을 다른 폴더에 풀었거나 중간에 다른 셀을 실행하면 `../../src/...` 같은 상대경로가 조용히 엉뚱한 곳을 가리키게 된다. 이 문서의 각 단계는 `%cd` 한 번으로 이동해두고 그 뒤 여러 셀을 잇달아 돌리는 걸 전제로 적혀 있지만, **경로 문제를 피하려면 매 셀을 독립된 터미널 명령처럼 취급해서 절대경로를 매번 명시하는 걸 권장한다.** 즉 `%cd` 대신:

```bash
!cd /content/bipedalwalker-td3/models/model1_g2-si-speed-s201 && bash run_train.sh
```

처럼 `cd`와 실행할 명령을 `&&`로 한 줄에 묶어서 쓰면, 이전 셀에서 어디에 있었는지와 무관하게 매번 같은 결과가 나온다. 아래 단계들에서 `%cd`가 나오면 이 방식으로 바꿔 써도 된다 — 특히 압축 해제 경로가 안내와 다르게 나왔다면(`/content/bipedalwalker-td3`가 아니라 `/content/extracted_archive/bipedalwalker-td3` 등) 반드시 이렇게 절대경로를 직접 확인하고 명시할 것.
